 ...
 * @property {string} type type of alert  // {(info|success|warning|error)}
...
export const useAlertStore = defineStore('alert', () => {
  ...
  const addAlert = (alert, routeOptions) => {
    ...
    if (!ALERT_TYPES.has(newAlert.type)) throw Error('Invalid alert type: ' + newAlert.type)

    // doubling up the timeout as we want to give the users time to click on it.
    if (newAlert.type === 'success' && routeOptions) {
      newAlert.timeout = routeOptions.to?.params?.id ? newAlert.timeout * 2 : newAlert.timeout
    }

    if (newAlert.timeout > 0)
      newAlert.timeoutId = setTimeout(() => removeAlertById(id), newAlert.timeout)
   ...
   * @param error
   * @param {number} timeout - Optional timeout for the alert to automatically dismiss (in milliseconds).
   * @returns {Alert|void} the newly created alert if one was created
   */
  const addErrorAlert = (message, error = null, timeout = 0) => {
    if (!error?.__CANCEL__) {
      // eslint-disable-next-line no-console
  ...
  const updateAlert = (alert) => {
    ...
    if (existingAlert) {
      ...
      alerts.value = alerts.value.map((a) => (a.id === alert.id ? updatedAlert : a))
      return updatedAlert
    } else {
